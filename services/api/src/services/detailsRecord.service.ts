...
import AttendenceRepository from '../repositories/attendance.repository.js';
...
class DetailsRecordService {
    ...
    private readonly attendanceRepository: AttendenceRepository;
    private readonly logger: DefaultLogger;
    constructor(detailsRecordRepository: DetailsRecordRepository, attendanceRepository: AttendenceRepository, logger: DefaultLogger) {
    ...
    async getDetailsRecordById(recordId: string, actorId: string, userRole: string) {
        try {
    ...
    async createDetailsRecord(recordData: Partial<IDetailsRecord>, actorId: string, userRole: string) {
        try {
    ...
    async updateDetailsRecord(
        ...
        try {
            const existingRecord = await this.detailsRecordRepository.findById(recordId);
            if (!existingRecord) {
                throw new Error("Không tìm thấy bản ghi theo ID");
            }

            const attendanceID = existingRecord?.attendanceId?.toString();
            const existingAttendance = await this.attendanceRepository.findById(attendanceID);
            if (!existingAttendance) {
                throw new Error("Không tìm thấy buổi điểm danh liên quan");
            }

            const existingAttendanceSchoolId = existingAttendance?.schoolId?.toString();

            // SuperAdmin => toàn quyền, bỏ qua các check
            if (userRole !== "SuperAdmin") {
                // SchoolAdmin chỉ được chỉnh trong trường của mình
                if (userRole === "SchoolAdmin" && schoolId !== existingAttendanceSchoolId) {
    ...
    async deleteDetailsRecord(recordId: string, schoolId: string, actorId: string, userRole: string) {
        try {
            const existingRecord = await this.detailsRecordRepository.findById(recordId);
            if (!existingRecord) {
                throw new Error("Không tìm thấy bản ghi theo ID");
            ...
            if (userRole !== "SuperAdmin") {
                ...
                if (userRole === "Teacher") {
                    ...
                    if (current > 1) {
                        throw new Error("Teacher chỉ được xóa bản ghi trong vòng 1 ngày kể từ ngày điểm danh");
                    }
                }
            }
