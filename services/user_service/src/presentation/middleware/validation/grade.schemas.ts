...
import { objectIdSchema, paginationQuerySchema, handleValidationErrors } from "./common.schemas";
...
export const createGradeSchema = [
  ...
  body('gradeGroup').optional().isString().withMessage('Grade group must be a string'),
  body('order').optional().isInt({ min: 1 }).withMessage('Order must be a positive integer'),
  body('active').optional().isBoolean().withMessage('Active must be a boolean'),
  body('ageRange').optional().isObject().withMessage('Age range must be an object'),
...
export const updateGradeSchema = [
  ...
  body('level').optional().isInt({ min: 1, max: 12 }).withMessage('Level must be between 1 and 12'),
  body('gradeGroup').optional().isString().withMessage('Grade group must be a string'),
  body('order').optional().isInt({ min: 1 }).withMessage('Order must be a positive integer'),
  body('active').optional().isBoolean().withMessage('Active must be a boolean'),
  body('ageRange').optional().isObject().withMessage('Age range must be an object'),
...
export const getGradesQuerySchema = [
  ...
  query('level').optional().isInt({ min: 1, max: 12 }).withMessage('Level must be between 1 and 12'),
...
export const getGradesEnhancedQuerySchema = [
  ...
  query('sortBy').optional().isIn(['name', 'level', 'order', 'createdAt', 'updatedAt']).withMessage('Invalid sort field'),
  query('sortOrder').optional().isIn(['asc', 'desc']).withMessage('Sort order must be asc or desc'),
  query('schoolId').optional().isMongoId().withMessage('Invalid school ID format'),
  query('level').optional().isInt({ min: 1, max: 12 }).withMessage('Level must be between 1 and 12'),
  query('active').optional().isBoolean().withMessage('Active must be a boolean'),
  query('search').optional().isLength({ min: 1, max: 100 }).withMessage('Search term must be 1-100 characters'),
  query('name').optional().isLength({ min: 1, max: 100 }).withMessage('Name filter must be 1-100 characters'),
  query('fields').optional().custom((value) => {
    if (typeof value === 'string') {
      const fields = value.split(',');
      const validFields = ['id', 'schoolId', 'name', 'level', 'gradeGroup', 'order', 'active', 'ageRange', 'description', 'createdAt', 'updatedAt'];
      const invalidFields = fields.filter(f => !validFields.includes(f.trim()));
...
export const bulkCreateGradesSchema = [
  body().isArray({ min: 1, max: 100 }).withMessage('Body must be an array with 1-100 items'),
  body('*.schoolId').notEmpty().withMessage('School ID is required for each grade'),
  body('*.name').notEmpty().isLength({ min: 1, max: 100 }).withMessage('Name must be 1-100 characters'),
  body('*.level').isInt({ min: 1, max: 12 }).withMessage('Level must be between 1 and 12'),
  body('*.gradeGroup').optional().isString().withMessage('Grade group must be a string'),
  body('*.order').optional().isInt({ min: 1 }).withMessage('Order must be a positive integer'),
...
export const bulkUpdateGradesSchema = [
  ...
  body('*.updates').notEmpty().isObject().withMessage('Updates object is required for each grade'),
  handleValidationErrors
];
