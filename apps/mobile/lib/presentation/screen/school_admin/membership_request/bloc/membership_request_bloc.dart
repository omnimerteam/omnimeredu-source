...
import 'membership_request_event.dart';
import 'membership_request_state.dart';

class MembershipRequestBloc
    extends Bloc<MembershipRequestEvent, MembershipRequestState> {
  ...
  Future<void> _onLoadRequests(
    ...
    Emitter<MembershipRequestState> emit,
  ) async {
    ...
    if (state is MembershipRequestLoaded) {
      ...
      try {
        final newStatus = await updateStatusMembershipRequest.call(
          UpdateStatusParams(id: event.requestId, status: event.newStatus),
        );
